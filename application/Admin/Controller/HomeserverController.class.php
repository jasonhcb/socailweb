...
class HomeserverController extends AdminbaseController {
	
    function _initialize()
    {
        parent::_initialize();
        $this->home_model =  M("homeserver");
        $this->term = M("terms");
        $this->city = M("areas");
    }
    ...
    public function addhome()
    {
        ...
        $term = $this->term->where("parent=3")->select();
    ...
    public function add_post()
    {
        if (IS_POST) {
            if (!empty($_POST['hos_img'])) {
                $data['home_img'] = sp_asset_relative_url($_POST['hos_img']);
            }
                $data['home_name'] = $_POST['hos_name'];
                $data['home_mobile'] = $_POST['hos_mobile'];
                $data['home_description'] = htmlspecialchars_decode($_POST['hos_des']);
                ...
                $data['home_address'] = $_POST['hos_add'];
                ...
                $result = $this->home_model->add($data);
                if ($result) {
                    $this->success("添加成功！");
                }else{
                    $this->error("添加失败！");
    ...
    public function allfind()
    {
        ...
        if (IS_POST && $_POST['tid']!='') {
            ...
            $result = $this->home_model->where($where)->select();
        }else{
            $result = $this->home_model->where($condition)->order('sort ASC')->select();
        }
        //分类
        $term = $this->term->where("parent=3")->select();
        $this->assign('term',$term);
    ...
    public function edit()
    {
        $id = I('get.id');
        $condition['id'] = $id;
        $result = $this->home_model->where($condition)->find();
    ...
    public function edit_post()
    {
        if (IS_POST) {
            if (!empty($_POST['hos_img'])) {
                ...
                $result = $this->home_model->where($condition)->save($data);
                if ($result) {
                    $this->success("修改成功！");
                }else{
                    $this->error("修改失败！");
    ...
    {
       $id = I('get.id');
       $result = $this->home_model->where("id=".$id)->delete();
       if ($result) {
           $this->success("删除成功");
       }else{
           $this->error("删除失败");
    ...
    {
        $id = I('get.id');
        $falg = $this->home_model->where("id=".$id)->find();
        if ($falg['status']== 0) {
           $result = $this->home_model->where("id=".$id)->setField('status',1);
        }else{
             $result = $this->home_model->where("id=".$id)->setField('status',0);
        }
        if ($result) {
            $this->success("修改成功");
        }else{
