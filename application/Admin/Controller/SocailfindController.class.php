...
class SocailfindController extends AdminbaseController{
	...
	public function order(){
        ...
        // echo $userid;
        if ($userid == 1) {
            $result = M("apoint")->select();
        }else{
            $result = M("apoint")->where("socail_id=".$user['socail_id'])->select();
        }
        // echo M("apoint")->getlastsql();
    ...
    {
       $id = I('get.id');
       $result = M('apoint')->where("id=".$id)->delete();
       if ($result) {
           $this->success("删除成功");
       }else{
           $this->error("删除失败");
