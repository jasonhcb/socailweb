 ...
 * 首页
 */
class BmfwController extends HomebaseController {
	...
	function _initialize()
    {
        ...
        $this->posts = M("posts");
        $this->term  = M("terms");
        $this->convenience = M("convenience");
    }
	...
	public function index() {
		...
		$id = I('get.id');
		if (I('get.term_id')) {
			$detail = $this->term->where("parent=".I('get.term_id'))->select();
			$this->assign("detail",$detail);
			if (I('get.detail')) {
				$condition['term_id'] = I('get.detail');
			}else{
				$condition['parent_id'] = I('get.term_id');
			}
		}
		...
		$fix = $this->convenience->where($condition)->limit($pages*10,10)->select();
		$allc = $this->convenience->where($condition)->select();
		$count = count($allc);
		$allpage = $count%10;
		// echo $allpage;
