 ...
 * 首页
 */
class WyuserController extends HomebaseController {
	...
	public function index() {
		...
		$term   = $this->term->where("parent=5")->select();
		$id = I('get.id');
		if (I('get.term_id')) {
		...
		$fix = $this->wy_user->where($condition)->limit($pages*10,10)->select();
		$allc = $this->wy_user->where($condition)->select();
		$count = count($allc);
		$allpage = $count%10;
		// echo $allpage;
