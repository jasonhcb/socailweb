...
<body>
	<div class="wrap js-check-wrap">
		<ul class="nav nav-tabs">
			<li class="active"><a href="javascript:;"><?php echo L('SOCAIL_MAIN');?></a></li>
			<li><a href="<?php echo U('Socail/add',array('term'=>empty($term['term_id'])?'':$term['term_id']));?>" target="_self"><?php echo L('SOCAIL_ADD');?></a></li>
		</ul>
		<form class="well form-search" method="post" action="<?php echo U('Property/main');?>">
<!-- 			分类： 
...
<!-- 					<td><input name="listorders[<?php echo ($vo["tid"]); ?>]" class="input input-order" type="text" size="5" value="<?php echo ($vo["sort"]); ?>" title="ID:<?php echo ($vo["id"]); ?>"></td> -->
					<td><span><?php echo ($vo["socail_name"]); ?></span></td>
...
<!-- 					<td>
					...
					<!-- <td><?php echo ($vo["creator_id"]); ?></td> -->
					<td><?php echo ($vo["create_time"]); ?></td>
					<td><?php echo ($status[$vo['status']]); ?><br><?php echo ($top_status[$vo['istop']]); ?><br><?php echo ($recommend_status[$vo['recommended']]); ?>
					</td>
					<td>
						<a href="<?php echo U('Socail/edit',array('term'=>empty($term['term_id'])?'':$term['term_id'],'id'=>$vo['id']));?>"><?php echo L('EDIT');?></a> | 
						<a href="<?php echo U('Socail/delete',array('term'=>empty($term['term_id'])?'':$term['term_id'],'id'=>$vo['tid']));?>" class="js-ajax-delete"><?php echo L('DELETE');?></a></td>
