...
<body>
	<div class="wrap js-check-wrap">
		<ul class="nav nav-tabs">
			<li class="active"><a href="<?php echo U('Property/user_admin');?>"><?php echo L('USER_ALL');?></a></li>
			<li><a href="<?php echo U('Property/user_add');?>"><?php echo L('USER_ADD');?></a></li>
		</ul>
    ...
    <?php if(is_array($result)): foreach($result as $k=>$vo): ?><!-- <?php echo ($k % 3); ?> -->
            ...
            <ul class="thumbnails">
              <?php if(is_array($result)): $i = 0; $__LIST__ = array_slice($result,$k,3,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$co): $mod = ($i % 2 );++$i;?><li class="span4">
                <div class="thumbnail">
                  <img alt="300x200" src="<?php echo sp_get_asset_upload_path($co['user_img']);?>" style="width: 300px; height: 200px;">
                  <div class="caption">
                    <h3><?php echo ($co["user_name"]); ?></h3>
                    <p><?php echo ($co["user_mobile"]); ?></p>
                    <!-- <p><?php echo ($co["hos_description"]); ?></p> -->
                    <p>
                      <a href="<?php echo U('Property/user_edit',array('id'=>$co['id']));?>" class="btn btn-primary">查看</a>
                      <a href="<?php echo U('Property/user_delete',array('id'=>$co['id']));?>" class="btn btn-danger">删除</a>
                      <a href="<?php echo U('Property/user_ban',array('id'=>$co['id']));?>" class="btn btn-info"><?php if($co['status']==0){ echo "显示";}else{echo "隐藏";} ?></a>
                      <a href="<?php echo U('Property/user_edit',array('id'=>$co['id']));?>" class="btn btn-success">修改</a>
                    </p>
                  </div>
                </div>
